...
class Linear(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_b = torch.sum(grad_output, dim=0).reshape(1,)
...
def libsvm(train_file_path, val_file_path, C, d):
    ...
    W = (np.reshape(sv_coef, (-1, 1)) * np.reshape(sv_feature, (-1, d))).sum(0)
    W = np.reshape(W, (1, d))
    b = (np.reshape(sv_labels, (-1, )) - (W * np.reshape(sv_feature, (-1, d))).sum(1)).mean()
